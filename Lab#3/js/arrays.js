...
const formatString = (receivedString, ...formatArguments) => {
  ...
  for (let i = 0; i < formatArguments.length; i++) {
    if (modifiedString.indexOf('{' + i + '}') !== -1) {
      ...
      modifiedString = modifiedString.replace('{' + i + '}', formatArguments[i])
    }
  }
...
// Function that receives an array of objects and an object => if all the fields in the object match any item in the array => remove it
...
const getFilteredObjects = (array, receivedObject) => {
  return array.filter(element => {
    ...
    Object.keys(receivedObject).forEach(key => {
      if (!element[key] || element[key] !== receivedObject[key]) {
        result = false
      }
    })

    return result // default return value is true
  })
}
